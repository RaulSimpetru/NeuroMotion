...
from NeuroMotion.MNPoollib.mn_utils import (
    plot_spike_trains,
    generate_emg_mu,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--res_path", default="./res", type=str, help="path of result folder"
    )
    ...
    msk.mov2len(muscle_labels=ms_labels)
    ms_lens = msk.muscle_lengths

    msk.len2params()
    changes = msk.param_changes
    ...
    ext = np.concatenate(
        (
            np.linspace(0, 0.8, round(fs * duration / 2)),
            np.linspace(0.8, 0, round(fs * duration / 2)),
        )
    )  # percentage MVC
    ...
    sos = butter(4, 800, btype="low", analog=False, fs=fs, output="sos")
    ...
    for sp in tqdm(
        ...
        sim = sosfiltfilt(sos, sim.reshape(-1, n_time_dim))
        muaps.append(
